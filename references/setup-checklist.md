...
> 这些是 bash 命令：**Windows 需要 WSL 或 Git Bash 才能跑**。
> 不同 AI 编程工具对"调用自定义脚本 / 路径"的支持本仓库未实测，请按你用的工具自己接一下。
...
- `--local`：只影响**当前这一个仓库**。**同一台电脑多人轮流用**（情景 3）、或一台机器多个 GitHub 身份时，每个仓库单独配 local，不要配 global。
  注意：`git worktree add` 出来的工作目录**共享同一份 config**，改 local 身份会影响所有 worktree；要在不同 worktree 用不同身份，见情景 4 的说明。
...
**stash 也别碰这些**：`git stash push` 默认会把工作区改动收起来，但如果你用了 `-u`（连未跟踪文件一起收），**`.env`、密钥、别人正在写的文件也会被收进去**。stash 前先 `git status` 看一眼，必要时按文件名加路径，别让 stash 误收 secrets。
