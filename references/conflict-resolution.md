...
新手最容易犯的错：看到冲突就跑一句 `git checkout --ours <文件>` 或 `--theirs <文件>`，**整个文件选一边**。
**90% 的情况下这是错的**——一个文件里可能既有你要保留的部分、也有朋友要保留的部分，直接选一边就把另一边的活丢了。
...
```bash
git add <解决好的文件>      # 一个一个加，或者 git add -A
```
...
- **合完发现跑不起来，想退回去**：刚合完还没 push 时，`git reset --hard ORIG_HEAD` 可以回到合并前（**但会丢掉你刚才解决冲突的工作**，先用 `git stash` 备份）。已经 push 了就别 reset，再提一个 commit 把问题修了。
